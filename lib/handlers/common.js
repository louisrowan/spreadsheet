...
const { COL_COUNT, ROW_COUNT, CELL_WIDTH, CELL_HEIGHT, TOOLBAR_HEIGHT } = require('../constants');
...
exports.scrollToNewCell = (cell, offset = 0) => {
    ...
    if (cellTop - (TOOLBAR_HEIGHT + CELL_HEIGHT) < windowTop) {
        window.scrollTo(windowLeft, cellTop - (TOOLBAR_HEIGHT + CELL_HEIGHT));
    }
    ...
    if (cellLeft - CELL_WIDTH < windowLeft) {
        window.scrollTo(cellLeft - CELL_WIDTH, windowTop);
    }
