...
describe('func cell out', () => {
    ...
    it('handle func cell output change', (done) => {
        ...
        // modify output cell
        Router({ state, type: 'cellMousedown', cell: summedCell });
        Router({ state, type: 'windowMouseup' });
        Router({ state, type: 'buttonClick', e: {
        ...
        expect(state.funcCellInput[cell1.id].includes(summedCell.id)).to.equal(false);
        expect(state.funcCellInput[cell2.id].includes(summedCell.id)).to.equal(false);
